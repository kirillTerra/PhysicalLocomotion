...
from rl_games.algos_torch.structure_transformers import (
    ...
    MLP, Transformer, BodyTransformer, BIBActorCritic, BodyLevelActor, BodyLevelCritic, ExpDecayTransformer
)
    ...
    class Network(NetworkBuilder.BaseNetwork):
        ...
        def _build_transformer(self, network_type): # Without tokenizer and detokenizer
            if network_type == 'transformer':
                return Transformer(self.dim_embeddings, self.dim_feedforward, self.num_heads, self.num_layers, save_attention=self.save_attention, save_path=self.attention_save_path )
            ...
            elif network_type == "exp_transformer":
                return ExpDecayTransformer(self.mapping, self.dim_embeddings, self.dim_feedforward, self.num_heads, self.num_layers, bias_type=self.bias_type, decay_rate=self.decay_rate)
            else:
        ...
        def load(self, params):
            ...
            self.decay_rate = params["transformer"].get('decay_rate', 0.5)

            self.nbodies = len(self.mapping['map'].keys())

            self.save_attention = params['transformer'].get('save_attention', False)
            self.attention_save_path = params['transformer'].get('attention_save_path', "/workspace/attention_masks")
...
class ExpDecayTransformerBuilder(TransformerBuilder):
    def __init__(self, **kwargs):
        centrality_degree = torch.Tensor([4, 1, 2, 3, 2, 2, 1, 2, 2, 1, 2, 2, 1, 2, 2, 1])
        shortest_path_matrix = torch.Tensor([[0, 1, 1, 2, 3, 4, 5, 3, 4, 5, 1, 2, 3, 1, 2, 3],
                                    [1, 0, 2, 3, 4, 5, 6, 4, 5, 6, 2, 3, 4, 2, 3, 4],
